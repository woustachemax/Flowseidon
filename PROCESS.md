...
## Day 6: Workflows UI, Premium Gating & Pagination

Built workflows page UI with reusable entity components. Created `EntityHeader` with microinteractions (Plus icon rotation, shimmer effects, cyan accents), `EntityContainer` with subtle gradient backgrounds, and `EntitySearch` with focus glow effects matching the design system.

Developed workflows feature with `WorkflowsHeader`, `WorkflowList`, and `WorkflowsContainer` using Suspense boundaries. Integrated `useCreateWorkflow` mutation with `trpc.useUtils()` for cache invalidation, Sonner toast notifications, and auto-navigation to created workflows.

Implemented premium feature gating with `premiumProcedure` middleware that throws `FORBIDDEN` errors when subscription is missing. Built `useUpgradeModal` hook to catch `TRPCClientError` and auto-trigger upgrade modal. Designed `UpgradeModal` with translucent cyan/red accents integrating Better Auth checkout flow.

Enhanced workflows endpoint with full pagination support including `hasNextPage`, `hasPreviousPage`, `isFirstPage`, `isLastPage`, `isEmpty`, `startIndex`, `endIndex`, `totalPages`, and `totalCount`. Integrated **nuqs** for type-safe URL state management with `parseAsInteger` and `parseAsString` parsers. Created `workflowsParams` schema (page, pageSize, search) with defaults from constants and `useWorkflowsParams` hook using `useQueryStates` for automatic URL sync with clear-on-default behavior.

Fixed sidebar responsive design: added `flex-shrink-0` to icons, `truncate` to labels, wrapped footer in `SidebarMenu` structure for proper alignment in collapsed/expanded states.


## Day 7: Search Debouncing, Pagination & Workflow Actions

Created `useEntitySearch` hook implementing debounced search pattern with local state management for instant UI feedback while preventing excessive API calls. The hook maintains `localSearch` state that updates immediately on input change, then debounces the actual params update by 500ms using `setTimeout` with proper cleanup. Implemented smart pagination reset that triggers whenever search terms change, ensuring users always start from page 1 with new search results. Added bidirectional sync with `useEffect` to handle external parameter changes from URL navigation or other sources.

Built complete `EntityPagination` component with Previous/Next buttons, page number display, and proper disabled states matching the design system. Created reusable entity components including `EntityList` for generic list rendering with built-in empty state handling, `EmptyView` with dashed borders and cyan glow effects, `LoadingView` for loading states, and `ErrorView` with rose-colored styling. Fixed duplicate header rendering issue in workflows page by removing redundant `WorkflowsHeader` from children since `WorkflowsContainer` already renders it via the `header` prop.

Implemented `WorkflowItem` component with shadcn dropdown menu providing Edit, Duplicate, and Delete actions for each workflow. Added hover effects revealing the three-dot menu button with cyan border highlights. Created `useDeleteWorkflow` and `useDuplicateWorkflow` hooks with proper cache invalidation using `trpc.useUtils()` and toast notifications via Sonner. Added corresponding `delete` and `duplicate` tRPC endpoints with user ownership validation - duplicate creates a copy with "(Copy)" suffix and navigates to the new workflow. Updated default page size from 5 to 20 items for better screen space utilization.